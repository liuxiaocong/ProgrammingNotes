...
下面来看一下刚刚说的一个很重要的方法**requestDisallowInterceptTouchEvent，**其实就是改变一个标志位，注意mParent.requestDisallowInterceptTouchEvent(disallowIntercept);他会调用自己的父控件的requestDisallowInterceptTouchEvent方法，然后一直往上调用
